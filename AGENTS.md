...
- **CM5 bring-up diagnostic:** Before starting services on DEXI-OS, run `python -m onboard.bringup_check`. It checks `rclpy`, `px4_msgs`, required GStreamer plugins, `/dev/ttyAMA2` access, and the configured command/video UDP ports; any failure is actionable hardware/software setup evidence, not a flight result.

...
- **CM5 ROS 2 bridge configuration:** The bridge CLI accepts `--bind-host`, `--command-port`, `--distance-topic`, and `--tick-period` for DEXI-OS deployment; defaults preserve the validated `0.0.0.0:5001`, `/fmu/out/distance_sensor`, and 50 Hz safety loop.
- **CM5 bring-up diagnostic:** Before starting services on DEXI-OS, run `python -m onboard.bringup_check`. It checks `rclpy`, `px4_msgs`, `ros2`, `micro_ros_agent`, required GStreamer plugins, `/dev/ttyAMA2` access, and the configured command/video UDP ports; any failure is actionable hardware/software setup evidence, not a flight result.
- **ROS distance validation:** `LatestDistanceSensor` accepts a PX4 distance reading only when it is finite and, when provided by the message, within the sensor's declared minimum/maximum range; invalid samples become the existing unsafe `NaN` state.
- **ROS bridge failure handling:** `Ros2SafetyBridge` records a CM5 forwarder-thread exception, waits for the service thread during close, destroys the ROS node and shuts down `rclpy`, then re-raises the failure from its entry point instead of silently continuing without PX4 heartbeats.
- **ROS bridge transport coverage:** The fake-node ROS bridge integration test now uses the production `UdpCommandSender`, so Mac packet encoding/sequencing and the CM5 ROS safety boundary are verified together rather than through test-only raw socket construction.
- **Vision freshness:** `LatestVisionPipeline` preserves the tracker-provided prediction age and adds transport/inference delay, so asynchronous vision cannot make a predicted or stale person appear freshly detected.
...
- **SITL cleanup simplification:** The full-stack verifier shares one small service-task shutdown helper, keeping its Mac/CM5 cleanup order explicit without duplicating exception-swallowing teardown blocks.
- **Camera pipeline liveness:** The CM5 video sender now surfaces a GStreamer child exit instead of appearing healthy while the camera stream is dead.
- **Mac video liveness:** The Mac control service now treats an RTP reader that remains empty for more than two seconds as a stalled video stream, sends its final zero command, and exits visibly rather than continuing blind.
- **Following direction:** Ordinary visual following only closes distance or holds; it never reverses from ambiguous image geometry. Reverse velocity remains limited to the explicit obstacle-driven `AVOIDING` override.
- **Voice ambiguity:** Conflicting non-safety voice intents are rejected rather than resolved by parser order; explicit hover/stop language remains a deliberate safe override.
- **Vision failure handling:** A vision-worker exception is now surfaced to the Mac control service, which follows its normal zero-command shutdown path instead of continuing without perceptual input.
- **Cognitive intent boundary:** Reactive control accepts only `State` enum values; malformed cognitive intent is rejected before it can overwrite the saved behavior state.
- **CM5 receiver startup:** UDP receiver binding is atomic: a bind/setup failure closes the new socket and leaves the receiver unstarted for a clean retry.
...
- **2026-07-28** — Performed the focused simplification milestone required by current steering: consolidated duplicate Mac/CM5 service-task teardown in `sim/offboard_full.py`. The full 122-test suite and real PX4 SITL full-stack run passed, observing `0.14 m/s` forward velocity, `-0.20 m/s` obstacle backoff, and a clean landing.
- **2026-07-28** — Added CM5 camera-pipeline liveness reporting and verified 123 tests plus a real RTP/H.264 loopback. The bring-up diagnostic confirms Mac-side GStreamer/UDP prerequisites are present while ROS 2, `px4_msgs`, `ros2`, `micro_ros_agent`, `libcamerasrc`, and `/dev/ttyAMA2` remain hardware/DEXI-OS gated.
- **2026-07-28** — Hardened the remaining Mac/CM5 safety boundaries: stalled RTP video, vision-worker errors, malformed cognitive states, and failed CM5 UDP binding now surface or fail safe rather than silently continuing. The final deterministic suite passed 127 tests; local command loopback again confirmed follow, obstacle backoff, and zero-on-shutdown. Full PX4 SITL continued to observe forward following, obstacle backoff, and clean landing after the video/vision changes.
...
- **2026-07-28** — Integrated the production YOLO/RTP path into PX4 SITL. The first run correctly exposed that synchronous inference blocked the reactive loop long enough to trip its 150 ms watchdog; `vision.latest.LatestVisionPipeline` now runs one inference in a worker and returns the latest estimate with explicit age, clearing on inference errors. The corrected SITL run completed the full profile, observed `-0.38 m/s` obstacle backoff telemetry, and landed cleanly. The full suite now contains 107 passing tests.
- **2026-07-28** — Added the Mac-side `control.udp_sender.UdpCommandSender` and routed both command loopback and adversarial UDP SITL through it, replacing verifier-only socket/packet construction. Local verification passed fresh forwarding, obstacle override, and shutdown zero; PX4 SITL passed heartbeat dropout, out-of-bounds suppression, `0.31 m/s` forward telemetry, `-0.19 m/s` backoff, and clean landing. The full suite now contains 110 passing tests.
- **2026-07-28** — Added `control.udp_control.UdpControlService`, the Mac-side lifecycle boundary that runs frame acquisition, the reactive tick, sequenced UDP transmission, and explicit zero shutdown. The local loopback now exercises that service through the real sender and CM5 envelope, verifying fresh follow output, obstacle override, and shutdown zero. A packet-drain timing race in the first verifier attempt was fixed by spacing simulated frames at the service cadence. The full suite now contains 113 passing tests.
- **2026-07-28** — Added `sim.offboard_full`, the first full-stack SITL verifier: real JPEG RTP/H.264, production YOLO/Kalman inference, Mac `UdpControlService`, CM5 `SafetyCommandService`, and PX4 MAVSDK forwarding run together. The hardened scenario completed the full profile, observed `0.14 m/s` maximum and `-0.40 m/s` obstacle backoff telemetry, and landed cleanly. This closes the software simulation integration gap; CM5 Wi-Fi/ROS serial and physical sensor validation remain hardware-gated.

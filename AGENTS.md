...
- **CM5 command service:** `onboard.command_service.SafetyCommandService` owns the receiver lifecycle, fixed-rate safety polling, approved-command forwarding, and one explicit zero command on shutdown. Call `start()` synchronously before the Mac sender can transmit; then schedule `run()`. The future DEXI-OS process supplies the hardware-specific forwarder and TOF callback.
- **CM5 ROS 2 bridge:** On DEXI-OS with `rclpy` and `px4_msgs` installed, run `python -m onboard.ros2_bridge`. It binds UDP before starting the service thread, subscribes to `/fmu/out/distance_sensor`, initializes the distance reading as unsafe until a message arrives, and publishes velocity-only PX4 setpoints with the injected ROS clock. The bridge is not executable on this Mac because ROS 2 is not installed here.
...
- **2026-07-28** — Added `onboard.ros2_bridge.Ros2SafetyBridge` and `python -m onboard.ros2_bridge`: the DEXI-side composition now binds the UDP receiver before a dedicated safety thread, injects the ROS/PX4 clock, subscribes to the standard distance-sensor output, and starts with an unsafe/no-reading value. Two fake-node lifecycle tests and the full 99-test suite pass; ROS 2 and physical serial/sensor validation remain hardware-gated.

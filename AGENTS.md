...
- **CM5 ROS 2 bridge:** On DEXI-OS with `rclpy` and `px4_msgs` installed, run `python -m onboard.ros2_bridge`. It binds UDP before starting the service thread, subscribes to `/fmu/out/distance_sensor`, initializes the distance reading as unsafe until a message arrives, and publishes velocity-only PX4 setpoints with the injected ROS clock. The bridge owns `/fmu/in/offboard_control_mode` and `/fmu/in/trajectory_setpoint`; do not run the stock `px4_offboard_manager` concurrently because it publishes position-mode setpoints. The bridge is not executable on this Mac because ROS 2 is not installed here.

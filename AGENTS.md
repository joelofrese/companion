...
- **CM5 command service:** `onboard.command_service.SafetyCommandService` owns the receiver lifecycle, fixed-rate safety polling, approved-command forwarding, and one explicit zero command on shutdown. The future DEXI-OS process supplies the hardware-specific forwarder and TOF callback.
...
- **UDP-to-SITL verification:** Run `python -m sim.offboard_udp` against PX4 SITL to exercise Mac packet generation → CM5 safety receiver → MAVSDK NED forwarding. The MAVSDK sink is a simulation stand-in; replace it with the DEXI-OS MAVLink/PX4 process when hardware arrives.

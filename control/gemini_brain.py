...
class GeminiRuntime:
    ...
    def __init__(
        ...
        self._follow_up_attempted = False
    ...
    async def _retry_action_follow_up(self, session):
        ...
        async with self._send_lock:
            ...
            self._follow_up_attempted = True
            self._response_in_flight = True
            print(
    ...
    def _finish_turn(self, response_started_s):
        ...
        if (
            ...
            and not self._follow_up_attempted
        ):
            self._follow_up_sent_at_s = time.monotonic()
...
def _system_instruction() -> str:
    ...
    return (
        ...
        "`hover`, or `speak` for each decision. Use the interface directly "
        "rather than an ordinary text response. Keep the user's situation or request "
        "active until it is complete or changed. For a concrete request, seeing a "
        "target is not completion: keep moving toward it when safe, verify the new "
        ...
        "move through clear space, and hover when the scene is unclear or unsafe. When "
        "the image shows a clear path and TOF is comfortably beyond the stop limit, "
        "prefer a short translation over repeated in-place turns. Use "
        "hover when no safe physical step is genuinely clear. Trust current image and "
        ...
        "requested target is centered and visible, keep it in view and make a short "
        "forward correction instead of turning just because the last action finished. "
        "If a visible subject is off-center but the path is clear, prefer a slow arc with "
        "small translation and yaw over a large in-place turn, then reobserve. After a "
        "turn, do not repeat the same in-place turn without a new visual reason; use "
        "the new image and heading to choose a correction, translation, or hover. Do not "
